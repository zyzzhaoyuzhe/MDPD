%% Stagewise EM for Crowdsourcing (Extended)

classdef CS_stageEM_Ext < MDPD
    properties
...
%         W % (output) diagonal matrix for label probablity
    ...
    properties
        ...
        missflag = false % missing data flag
        missperc % missing percentage for each worker ( m);
    end
    %%
    methods
        %% constructor
        function obj = CS_stageEM_Ext(m,n,k,r)
            obj@MDPD(m,n,k,r);
        end
        %%
        function [obj,disp] = learn(obj)
            ...
            obj.C_tmp(obj.C_tmp<delta) = delta;
            ...
            % E-M loop
            stop = false;
            % missing data mode: assign missperc
            obj.missperc = obj.C_tmp(end,1,:);
            obj.missperc = squeeze(obj.missperc);
...
%             close(h);
        ...
        function obj = split_multi(obj,i,j,k)
            ...
            if obj.missflag
                H = stageEM_Hessian_ext(obj.z,obj.C_tmp,obj.W_tmp,obj.activeset,i,j,k);
            else
        ...
        function obj = EM(obj,activeset)
            ...
            elseif obj.Mstep_switch == 2
                % partial M step
                [obj.C_tmp,obj.W_tmp] = stageEM_Mstep_partial(obj.z,obj.C_tmp,post,activeset);
            end
            
            % missing data flag (the last label is the missing data label
            % which should be the same accross components
            if obj.missflag
                for i = 1:obj.m
                    obj.C_tmp(end,:,i) = obj.missperc(i);
                end
            end
